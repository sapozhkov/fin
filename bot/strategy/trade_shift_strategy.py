...
class TradeShiftStrategy(TradeAbstractStrategy):
    ...
    def update_orders_status(self):
        ...
        for order_id, order in self.bot.active_buy_orders.copy().items():
            if order_id not in active_order_ids:
                ...
                if is_executed and order_state:
                    ...
                    self.bought_price = self.bot.get_order_avg_price(order_state)
        ...
        for order_id, order in self.bot.active_sell_orders.copy().items():
            if order_id not in active_order_ids:
                ...
                if is_executed and order_state:
                    self.bot.apply_order_execution(order_state)
                    self.sold_price = self.bot.get_order_avg_price(order_state)
                self.bot.remove_order_from_active_list(order)

        if self.sold_price or self.bought_price:
            self.bot.cancel_active_orders()
    ...
    def place_buy_orders(self):
        cur_order_prices = self.bot.get_existing_buy_order_prices()
        cur_orders_cnt = len(cur_order_prices)
        need_to_add = self.bot.config.step_set_orders_cnt - cur_orders_cnt
        ...
        if self.sold_price:
            ...
            cur_set_order_price = math.floor(current_price / self.bot.config.step_size) * self.bot.config.step_size
        ...
        for i in range(need_to_add):
            ...
            if self.bot.get_current_step_count() + i > self.bot.config.step_max_cnt:
                break

            # номер шага для заявки относительно количества в портфеле
            current_step_cnt = max(0, self.bot.get_current_step_count() + i)

            # вычисление сдвига
            price_shift = self.bot.config.step_size * (1 + current_step_cnt * self.bot.config.step_size_shift)

            # смещение текущего значения цены
            cur_set_order_price = self.bot.round(cur_set_order_price - price_shift)

            # выставление заявки
            self.bot.buy_limit(cur_set_order_price, self.bot.config.step_lots)

    def place_sell_orders(self):
        cur_order_prices = self.bot.get_existing_sell_order_prices()
        cur_orders_cnt = len(cur_order_prices)
        need_to_add = self.bot.config.step_set_orders_cnt - cur_orders_cnt
        ...
        if self.bought_price:
            ...
            if not current_price:
                ...
                return
            # cur_set_order_price = current_price - self.config.step_size / 2  # тут "-", чтобы разница в шаг как раз была
            cur_set_order_price = math.ceil(current_price / self.bot.config.step_size) * self.bot.config.step_size

        # добавляем заявки со сдвигом учитывая сколько лотов будет в портфеле при покупке
        for i in range(need_to_add):
            # не надо превышать максимального числа заявок
            min_steps = -self.bot.config.step_max_cnt if self.bot.config.majority_trade else 0
            if self.bot.get_current_step_count() - i <= min_steps:
                break

            # номер шага для заявки относительно количества в портфеле
            current_step_cnt = abs(min(0, self.bot.get_current_step_count() - i))
            ...
            # выставление заявки
            self.bot.sell_limit(cur_set_order_price, self.bot.config.step_lots)
